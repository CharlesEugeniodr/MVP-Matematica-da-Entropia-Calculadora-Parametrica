...
const Controls = (() => {
  ...
  const PARAM_DEFS = [
    ...
    { group: 'Macro-Direcionadores', params: [
      { key: 'gov_policy', label: 'Governança Global', min: 0, max: 1, step: 0.01 },
      { key: 'tech_innovation', label: 'Inovação Tecnológica', min: 0, max: 1, step: 0.01 },
      { key: 'emissions', label: 'Intensidade de Emissões', min: 0, max: 1, step: 0.01 },
      { key: 'land_use', label: 'Uso Predatório do Solo', min: 0, max: 1, step: 0.01 },
      { key: 'pollution', label: 'Nível de Poluição', min: 0, max: 1, step: 0.01 },
      { key: 'inequality', label: 'Desigualdade (Gini)', min: 0, max: 1, step: 0.01 },
    ]}
  ];
